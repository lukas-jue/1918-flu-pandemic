...
%\usepackage{amsmath}
\usepackage{caption}
%\usepackage[small]{titlesec} % adjust size of section headings

% fix wrong first quotation marks. Usually you would need to write ``quote'' to have well-typesetted quotes.
...
%\renewcommand{\familydefault}{cmr} % Computer Modern. alternative: cmss, cmr
\usepackage{palatino} %Font Palatino, check the right margin, might be overfull
...
\pdfminorversion=7
\bibliographystyle{agsm} %agsm BibTeX style for references
...
\captionsetup[figure]{labelfont={bf, small},textfont={it, small}, format=hang}
\captionsetup[lstlisting]{labelfont={bf, small},textfont={it, small}, format=hang}
\interfootnotelinepenalty=100000 %prevents footnotes from breaking across pages
...
{\large{\bf Economic Consequences of the 1918 Flu Pandemic:\\
		Reviewing Evidence from the Historical Precedent of Covid-19}}  \\\vspace{1.5cm}

\textbf{Lukas J\"urgensmeier} \vspace{1.5cm}
...
The current Covid-19 pandemic comes with severe medical and economic consequen\-ces.
...
Since the prime working age population's health was severely negatively affected by the pandemic \citep{taubenberger1918InfluenzaMother2006}, industries struggled with an acute shortage of labor supply.
...
\appendix
\section{Appendix}
\addtocontents{toc}{\protect\setcounter{tocdepth}{1}} % prevents the following section from showing up in TOC
\subsection{R Code}
\addtocontents{toc}{\protect\setcounter{tocdepth}{2}} % resets default TOC settings

\input{code/lst_cfr.tex}

\input{code/lst_cfr_plot.tex}
...
Frankfurt am Main, June 11, 2020
